...
# --- Configuration ---
# Default output is now the current directory
DEFAULT_OUTPUT_DIR = "."
...
def download_video(args_tuple: Tuple[str, str, str, Optional[str], bool]) -> Tuple[str, bool, Optional[str]]:
    ...
    try:
        if ua:
            ...
            user_agent = "Mozilla/5.0" # Simpler fallback

        # Construct the full output path template
        # os.path.abspath ensures we handle "." correctly relative to the CWD
        ...
        ydl_opts = {
            ...
            } if 'writethumbnail' in ydl_opts and ydl_opts['writethumbnail'] else None,
        }
...
def main(urls: List[str], quality: str, output_dir: str, processes: int, cookiefile: Optional[str], quiet_ydl: bool):
    ...
    # Resolve output_dir to an absolute path for clarity in logs
    ...
    # Create output directory if it doesn't exist (handles ".")
    try:
        # exist_ok=True prevents error if the directory already exists (like ".")
        os.makedirs(abs_output_dir, exist_ok=True)
        if output_dir != ".": # Only log creation if it's not the current dir
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output",
        type=str,
        default=DEFAULT_OUTPUT_DIR, # Default changed to "."
    ...
    if args.urls:
        ...
        try:
            # Ensure file path is handled correctly, maybe relative to CWD
    ...
    # Resolve potential relative path for cookie file
    cookie_path = None
    if args.cookiefile:
        cookie_path = os.path.abspath(args.cookiefile)
        if not os.path.exists(cookie_path):
             log.warning(f"Specified cookie file does not exist: {cookie_path}")
             # Proceed without cookie file, ydl_opts handles non-existent file warning later
